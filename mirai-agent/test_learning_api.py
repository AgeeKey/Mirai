...
╚══════════════════════════════════════════════════════════════════════╝
    ...
    try:
        ...
        results.append(
            test_result("Create task via POST /api/nasa/learn", False, str(e))
        )
        task_id = None
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            results.append(
                test_result("Get task status", False, f"Status: {response.status_code}")
            )
    except Exception as e:
        ...
        while time.time() - start_time < max_wait:
            ...
            if response.status_code == 200:
                ...
                if status != last_status or progress != last_progress:
                    ...
                    print(
                        f"   [{elapsed:5.1f}s] Status: {status:10s} | Progress: {progress:3d}%"
                    )
                    last_status = status
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            results.append(
                test_result(
                    "Create second task", False, f"Status: {response.status_code}"
                )
            )
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            results.append(
                test_result(
                    "Task manager stats", False, f"Status: {response.status_code}"
                )
            )
    except Exception as e:
