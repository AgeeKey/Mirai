...
🎯 ТВОЯ ЗАДАЧА: Выбери ОДНУ конкретную проблему и СРАЗУ РЕШИ ЕЁ используя инструменты!

ДОСТУПНЫЕ ИНСТРУМЕНТЫ:
• execute_code - выполнить код (Python, JS, C++, Go, Rust, Bash)
• write_file - создать/изменить файл
• read_file - прочитать файл
• run_command - выполнить команду в терминале
• github_action - работа с GitHub (create_issue, list_repos, search_repos)
• database_query - работа с БД (SQLite, PostgreSQL, MongoDB, Redis)
• search_web - поиск в интернете

ПРИМЕРЫ ДЕЙСТВИЙ:
1. Если CI/CD падает - используй read_file чтобы посмотреть логи ошибок, потом исправь их
2. Если мало тестов - используй write_file чтобы создать новые тесты ПРЯМО СЕЙЧАС
3. Если нужна инфо - используй search_web или github_action

⚠️ ВАЖНО: НЕ пиши "Я проанализирую..." или "Я предлагаю...".
✅ СРАЗУ ИСПОЛЬЗУЙ ИНСТРУМЕНТЫ! Например: read_file("tests/test_example.py"), потом write_file с новым тестом!

Если задача требует ручной работы от человека - скажи "НУЖЕН_ЧЕЛОВЕК: да" и объясни что именно.
"""

        # ВАЖНО: max_iterations=10 чтобы МИРАЙ могла выполнить сложные задачи!
