...
class CodeQualityAnalyzer:
    ...
    def analyze(self, code: str, filepath: str = "temp.py") -> QualityMetrics:
        ...
        if complexity_metrics['maintainability'] < self.min_maintainability:
            issues.append(f"Low maintainability: {complexity_metrics['maintainability']:.1f}")
    ...
    def _analyze_complexity(self, code: str) -> dict:
        ...
        if RADON_AVAILABLE:
            try:
                ...
                avg_complexity = sum(c.complexity for c in complexity) / max(len(complexity), 1) if complexity else 1
