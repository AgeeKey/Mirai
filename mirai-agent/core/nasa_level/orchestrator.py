...
class NASALearningOrchestrator:
    ...
    def __init__(self):
        ...
        self.learning_engine = AdvancedLearningEngine(
            self.ai_agent, self.sandbox, self.quality_analyzer
        )
    ...
    def learn_technology(
        ...
        if result.success:
            ...
            self.metrics.record_learning(
                technology=technology, success=False, errors=result.errors
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "command",
        choices=["learn", "queue", "status", "report", "search"],
        help="Command to execute",
    )
    parser.add_argument("--tech", help="Technology to learn")
    parser.add_argument(
        "--depth", default="basic", choices=["basic", "intermediate", "advanced"]
    )
    parser.add_argument(
        "--priority", default="normal", choices=["critical", "high", "normal", "low"]
    )
    ...
    if args.command == "learn":
        ...
        for r in results:
            print(
                f"  - {r.technology} v{r.version} (grade: {r.quality_grade}, {r.proficiency:.1%})"
            )
