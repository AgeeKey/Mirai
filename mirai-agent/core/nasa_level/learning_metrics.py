...
PROFICIENCY_SCORE = Gauge(
    "nasa_proficiency_score", "Current proficiency score for technology", ["technology"]
)

QUALITY_SCORE = Gauge(
    "nasa_quality_score", "Current quality score for technology", ["technology"]
)
...
ACTIVE_LEARNING_TASKS = Gauge(
    "nasa_active_learning_tasks", "Number of currently active learning tasks"
)

KNOWLEDGE_BASE_SIZE = Gauge(
    "nasa_knowledge_base_size", "Total number of learned technologies"
)
...
class LearningMetrics:
    ...
    def record_learning(
        ...
        LEARNING_ATTEMPTS.labels(technology=technology, depth=depth).inc()

        if success:
            LEARNING_SUCCESS.labels(technology=technology, grade=quality_grade).inc()

            LEARNING_DURATION.labels(technology=technology, depth=depth).observe(
                duration
            )

            PROFICIENCY_SCORE.labels(technology=technology).set(proficiency)

            # Map grade to score
            grade_map = {"A": 1.0, "B": 0.85, "C": 0.7, "D": 0.55, "F": 0.4}
            quality_score = grade_map.get(quality_grade, 0.4)

            QUALITY_SCORE.labels(technology=technology).set(quality_score)

        else:
            reason = errors[0] if errors else "unknown"
            LEARNING_FAILURES.labels(
                technology=technology, reason=reason[:50]  # Limit label length
            ).inc()
        ...
        if success:
            ...
            tech_stats["best_proficiency"] = max(
                tech_stats["best_proficiency"], proficiency
            )
        else:
    ...
    def get_average_duration(self, technology: str = None) -> float:
        ...
        if technology:
            ...
            total_duration = sum(e.duration for e in self.events if e.success)
    ...
    def get_top_technologies(self, limit: int = 10) -> List[Dict]:
        ...
        tech_list.sort(
            key=lambda x: (x["proficiency"], x["success_rate"]), reverse=True
        )
    ...
    def get_summary(self) -> Dict:
        ...
        avg_proficiency = (
            ...
            if any(e.success for e in recent_events)
            else 0.0
        )
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            f"  Success: {summary['total_success']} ({summary['overall_success_rate']:.1%})"
        )
                    ...
                    for line in f:
                        if line.strip():
                            ...
                            if event.success:
                                ...
                                tech_stats["best_proficiency"] = max(
                                    tech_stats["best_proficiency"], event.proficiency
                                )
                            else:
                                tech_stats["failures"] += 1
