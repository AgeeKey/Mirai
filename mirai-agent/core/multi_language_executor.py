...
class MultiLanguageExecutor:
    ...
    async def _execute_javascript(self, code: str) -> Dict[str, str]:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.js', delete=False, dir=self.working_dir) as f:
            f.write(code)
            filepath = f.name
    ...
    async def _execute_typescript(self, code: str) -> Dict[str, str]:
        ...
        if not shutil.which('ts-node'):
            # Пробуем через npx
            with tempfile.NamedTemporaryFile(mode='w', suffix='.ts', delete=False, dir=self.working_dir) as f:
                f.write(code)
                filepath = f.name
    ...
    async def _execute_c(self, code: str) -> Dict[str, str]:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.c', delete=False, dir=self.working_dir) as f:
            f.write(code)
            source_file = f.name
        ...
        try:
            # Компиляция
            compile_result = await self._run_command(['gcc', source_file, '-o', executable])
    ...
    async def _execute_cpp(self, code: str) -> Dict[str, str]:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.cpp', delete=False, dir=self.working_dir) as f:
            f.write(code)
            source_file = f.name
        ...
        try:
            # Компиляция
            compile_result = await self._run_command(['g++', source_file, '-o', executable, '-std=c++17'])
            if not compile_result['success']:
                return {
                    'success': False,
                    'output': compile_result['output'],
    ...
    async def _execute_go(self, code: str) -> Dict[str, str]:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.go', delete=False, dir=self.working_dir) as f:
            f.write(code)
            filepath = f.name
    ...
    async def _execute_rust(self, code: str) -> Dict[str, str]:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.rs', delete=False, dir=self.working_dir) as f:
            f.write(code)
            source_file = f.name
        ...
        try:
            # Компиляция
            compile_result = await self._run_command(['rustc', source_file, '-o', executable])
            if not compile_result['success']:
                return {
                    'success': False,
                    'output': compile_result['output'],
    ...
    async def _execute_bash(self, code: str) -> Dict[str, str]:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.sh', delete=False, dir=self.working_dir) as f:
            f.write(code)
            filepath = f.name
...
if __name__ == '__main__':
    async def test():
        ...
        # JavaScript
        print("\n📜 JavaScript:")
        result = await executor.execute_code('console.log("Hello from Node.js!")', 'javascript')
        print(f"   {result}")
