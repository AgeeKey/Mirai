    ...
    def analyze_file(self, file_path: str) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'has_docstrings': any(isinstance(node, (ast.FunctionDef, ast.ClassDef)) and ast.get_docstring(node) for node in ast.walk(tree)),
...
class AICodeImprover:
    ...
    def generate_improvement(self, file_path: str, issues: List[Dict]) -> Optional[Dict]:
        ...
        try:
            ...
            try:
                ...
                result = subprocess.run(
                    ['python3', '-c', f'import sys; sys.path.insert(0, "/root/mirai/mirai-agent"); exec(open("{tmp_path}").read())'],
                    capture_output=True,
                    timeout=5,
    ...
    def analyze_codebase(self) -> Dict:
        ...
        for file_path in files:
            ...
            if 'error' not in analysis:
                ...
                high_priority_issues += sum(1 for i in analysis['issues'] if i.get('severity') == 'high')
        
        logger.info(f"✅ Проанализировано {len(results)} файлов")
        logger.info(f"📊 Найдено {total_issues} проблем ({high_priority_issues} критичных)")
    ...
    def propose_improvements(self, analysis: Dict) -> List[Dict]:
        ...
        for file_data in files_with_issues[:3]:
            ...
            if improvement:
                ...
                logger.info(f"💡 Улучшение для {os.path.basename(file_data['file'])}: {improvement['solution'][:60]}...")
