...
result = subprocess.run(
    ["systemctl", "is-active", "mirai"], capture_output=True, text=True
)
...
# Uptime
result = subprocess.run(
    ["systemctl", "show", "mirai", "--property=ActiveEnterTimestamp"],
    capture_output=True,
    text=True,
)
...
# Memory
result = subprocess.run(
    ["systemctl", "show", "mirai", "--property=MemoryCurrent"],
    capture_output=True,
    text=True,
)
if result.returncode == 0:
...
if errors:
    ...
    for msg, count in sorted(unique_errors.items(), key=lambda x: x[1], reverse=True)[
        :5
    ]:
        print(f"      [{count}x] {msg}")
else:
...
try:
    ...
    if not hasattr(ltm, "get_recent_achievements"):
        issues.append(
            "❌ КРИТИЧНО: LongTermMemory.get_recent_achievements() не существует"
        )
except:
...
if db_path.exists():
    ...
    if xp < 100:
        issues.append(
            f"⚠️ ПРОБЛЕМА: Personality System почти не используется (XP: {xp:.0f})"
        )
