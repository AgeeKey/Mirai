...
async def test_databases():
    ...
    # Тест SQLite
    print("\n📦 SQLite:")
    result = await db.sqlite_query(
        "SELECT datetime('now') as current_time, 'MIRAI' as agent"
    )
    ...
    if "error" not in result[0]:
        print(
            f"   ✅ Подключено! Версия: {result[0].get('version', 'unknown')[:50]}..."
        )
    else:
    ...
    # Тест MongoDB
    print("\n🍃 MongoDB:")
    result = await db.mongodb_insert(
        "test", {"message": "Hello from MIRAI!", "timestamp": "now"}
    )
    ...
    for db_name, status in info.items():
        ...
        print(
            f"{status_icon} {db_name.upper()}: {status['status']} - {status['details']}"
        )
    print(f"{'='*70}")
