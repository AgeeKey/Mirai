...
from modules.agent.advisor import SignalAdvisor
from modules.agent.policy import PolicyEngine

from .async_loop import AsyncTradingLoop, TradingSignal, TradingState
...
class AIIntegratedTradingLoop(AsyncTradingLoop):
    ...
    async def _initialize_traditional_components(self):
        ...
        try:
            # Initialize advisor (uses env/config internally)
    ...
    async def _process_ai_recommendations_continuously(self):
        ...
        pending_tasks: Dict[str, tuple[str, datetime]] = {}  # task_id -> (symbol, submission_time)
            ...
            try:
                ...
                for task_id, (symbol, submission_time) in pending_tasks.items():
                    ...
                    if recommendation:
                        ...
                        try:
                            self.ai_recommendation_queue.put_nowait(recommendation)
            ...
            while not self.ai_recommendation_queue.empty():
                try:
                    ...
                    if rec.symbol == symbol:
                        ...
                        self.ai_recommendation_queue.put_nowait(rec)
