...
class BrokerConnector(ABC):
    ...
    def __init__(self, credentials: BrokerCredentials):
        ...
        # Client-side protocol handle (opaque for typing)
        self.ws_connection: Optional[object] = None
...
class BinanceConnector(BrokerConnector):
    ...
    async def disconnect(self):
        ...
        if self.ws_connection:
            ...
            await _cast(_Any, self.ws_connection).close()
        self.connected = False
...
async def test_broker_connectors():
    ...
    if order_id is None:
        raise RuntimeError("Order ID is None after placement")
    retrieved_order = await active_connector.get_order("BTCUSDT", order_id)
    print(f"Полученный ордер: {asdict(retrieved_order)}")

    # Отмена ордера
    cancelled = await active_connector.cancel_order("BTCUSDT", order_id)
