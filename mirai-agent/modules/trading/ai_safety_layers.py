...
# Опциональная проверка доступности OpenAI без импорта (избегаем F401)
...
class MarketAnalysisAI:
    ...
    async def _openai_market_analysis(self, market_data: Dict) -> Dict:
        ...
        try:
            # Здесь может быть обращение к LLM провайдеру
...
class LLMSafetyValidator:
    ...
    async def _basic_safety_checks(self, context: TradingContext) -> Dict[str, Any]:
        ...
        if context.position_size_usd > max_position:
            ...
            issues.append(f"Позиция {context.position_size_usd} превышает лимит {max_position}")

        # Проверка стоп-лосса
        if not context.signal.get('stop_loss'):
            checks["stop_loss_present"] = False
            issues.append("Отсутствует стоп-лосс")
        ...
        if required_margin > available_balance:
            ...
            issues.append("Недостаточно средств для маржи")
...
class AISafetySystem:
    ...
    async def validate_trade_execution(self, signal: Dict, market_data: Dict,
                                     account_status: Dict, risk_metrics: Dict) -> Tuple[bool, Optional[SafetyCheck]]:
