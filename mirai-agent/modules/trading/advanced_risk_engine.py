...
class AdvancedRiskEngine:
    ...
    def __init__(self, initial_balance: float = 10000.0, limits: Optional[RiskLimits] = None):
        ...
        self.db_path = os.environ.get("MIRAI_DB_PATH", "data/state/mirai.db")
        self.init_database()
    ...
    async def calculate_risk_metrics(self) -> RiskMetrics:
        ...
        if len(self.closed_positions) > 5:
            ...
            metrics.portfolio_volatility = float(np.std(returns)) if returns else 0.0
    ...
    async def log_risk_event(self, event_type: str, description: str, severity: str, position_id: Optional[str] = None):
