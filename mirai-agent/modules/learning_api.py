...
class LearningTaskManager:
    ...
    def __init__(self, max_concurrent: int = 2):
        ...
        logger.info(
            f"🎯 LearningTaskManager initialized (max_concurrent={max_concurrent})"
        )
    ...
    def add_task(
        ...
        self._emit_event(
            task_id,
            "created",
            {"task_id": task_id, "technology": technology, "status": "pending"},
        )
    ...
    async def _execute_task(self, worker_id: int, task: LearningTask):
        ...
        """
        logger.info(
            f"🎓 Worker {worker_id} executing: {task.task_id} - {task.technology}"
        )
        ...
        try:
            ...
            logger.info(
                f"✅ Task completed: {task.task_id} - {task.technology} "
