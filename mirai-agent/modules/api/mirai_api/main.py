...
try:
    from modules.performance.optimization import (
    ...
    async def broadcast(self, message: str):
        for connection in self.active_connections:
            try:
                ...
                # Удаляем соединение при ошибке отправки
...
async def startup_event():
    """Инициализация приложения и performance-системы (если доступна)."""
    ...
    # Прочие подготовительные действия можно добавить здесь
...
def get_db_connection() -> Optional[sqlite3.Connection]:
    """Возвращает подключение к БД (или None, если файл отсутствует)."""
    db_path = os.environ.get("MIRAI_DB_PATH", "data/state/mirai.db")
...
# (Дополнительные lifecycle-события перенесены в единые startup/shutdown выше)
