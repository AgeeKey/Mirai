...
class AlertManager:
    ...
    def __init__(self, telegram_notifier=None, email_config: Optional[Dict[str, Any]] = None):
    ...
    async def check_portfolio_risk(self, portfolio_data: Dict[str, Any]):
        ...
        _unrealized_pnl = portfolio_data.get('unrealized_pnl', 0)
    ...
    async def check_position_risk(self, symbol: str, position_data: Dict[str, Any]):
        ...
        _size = abs(position_data.get('size', 0))
        _unrealized = position_data.get('unrealized_pnl', 0)
        _entry_price = position_data.get('entry_price', 0)
        _current_price = position_data.get('current_price', 0)
    ...
    async def check_performance_metrics(self, performance_data: Dict[str, Any]):
        ...
        _sharpe_ratio = performance_data.get('sharpe_ratio', 0)
        _daily_trades = performance_data.get('daily_trades', 0)
    ...
    async def check_ai_model_performance(self, model_data: Dict[str, Any]):
        ...
        _last_update = model_data.get('last_update')
    ...
    def _format_telegram_message(self, alert: Alert) -> str:
        ...
        ts = alert.timestamp or datetime.now()
        message += f"⏰ Time: `{ts.strftime('%Y-%m-%d %H:%M:%S')}`"
    ...
    async def _send_email_alert(self, alert: Alert):
        ...
        # Create message
        msg = MIMEMultipart()
        msg['From'] = cast(str, sender_email)
        msg['To'] = ', '.join(cast(List[str], recipient_emails))
        ...
        # Send email
        with smtplib.SMTP(cast(str, smtp_server), int(smtp_port)) as server:
            server.starttls()
            server.login(cast(str, sender_email), cast(str, sender_password))
    ...
    def get_alert_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_24h': len([
                ...
                if (a.timestamp or datetime.now()) > datetime.now() - timedelta(days=1)
            ])
        }
